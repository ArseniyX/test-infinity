...
const ContactsTable = () => {
  const fetchContactsByPage = async ({ pageParam = 1 }) => {
    const { data } = await axios.get('/api/contacts', { params: { page: pageParam } });
    return data;
  };
  ...
  const memoizedData = useMemo(() => isLoading ? [] : data.pages.flat(), [isLoading, data]);
      ...
      <TableRow {...row.getRowProps({ style })}>
        {row.cells.map((cell) => (
            <TableCell
              ...
              sx={cell.column.id === 'emailAddress' ? { overflowX: 'hidden', textOverflow: 'ellipsis' } : undefined}
              {...cell.getCellProps()}
            >
              {cell.render('Cell')}
            </TableCell>
  ...
  return (
    ...
    <TableContainer sx={{ height: '100vh' }}>
      <Table sx={{ height: '100%' }} {...getTableProps()}>
        <TableHead>
          {headerGroups.map((headerGroup, i) => <HeaderGroup key={`header-group-${i}`} headerGroup={headerGroup} />)}
        </TableHead>
